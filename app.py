...
def reset_statistics():
    try:
        ...
        if "GITHUB_TOKEN" in st.secrets:
            ...
            repo = g.get_repo("Yorchip/AUTOESCUELA")
            try:
                contents = repo.get_contents("estadisticas.xlsx")
                repo.delete_file(
                    path="estadisticas.xlsx",
                    message="🗑️ Reinicio de estadísticas acumuladas",
                    sha=contents.sha
                )
            except Exception:
                pass
        st.success("✅ ¡Estadísticas reiniciadas con éxito!")
    except Exception as e:
        st.error(f"Error al reiniciar estadísticas: {e}")
...
def update_statistics(pregunta_data, correcto):
    ...
    if correcto:
        ...
        estadisticas[clave]['aciertos_consecutivos'] = 0
...
# --- VISTA: ESTADÍSTICAS ---
    ...
    if not stats:
        ...
        st.divider()
        if st.button("🗑️ Reiniciar todas las estadísticas", type="primary"):
            reset_statistics()
            st.rerun()
